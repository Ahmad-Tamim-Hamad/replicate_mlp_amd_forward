...
def read_flag():
    ...
    parser.add_argument(
        "--data-set", default=DATA_SET, type=str, help="which data set you are chosing"
    )
    # Model Architectural Params
    parser.add_argument(
        "--skip-connection",
        type=bool,
        default=SKIP_CONNECTION,
        help="The boolean flag indicates whether use skip connections",
    )
    parser.add_argument(
        "--use-conv",
        type=bool,
    ...
    parser.add_argument(
        "--linear", type=list, default=LINEAR, help="The fc layers units"
    )
    parser.add_argument(
        "--conv-out-channel",
        type=list,
        default=CONV_OUT_CHANNEL,
    ...
    parser.add_argument(
        ...
        type=list,
        default=CONV_STRIDE,
        help="The strides of your 1d conv",
    )
    # Optimization params
    parser.add_argument(
        "--optim",
        default=OPTIM,
        type=str,
        help="the type of optimizer that you want to use",
    )
    parser.add_argument(
        "--reg-scale",
        type=float,
        default=REG_SCALE,
        help="#scale for regularization of dense layers",
    )
    parser.add_argument(
        "--x-range", type=list, default=X_RANGE, help="columns of input parameters"
    )
    parser.add_argument(
        "--y-range", type=list, default=Y_RANGE, help="columns of output parameters"
    )
    parser.add_argument(
        "--batch-size", default=BATCH_SIZE, type=int, help="batch size (100)"
    )
    parser.add_argument(
        "--eval-step", default=EVAL_STEP, type=int, help="# steps between evaluations"
    )
    parser.add_argument(
        "--train-step",
        default=TRAIN_STEP,
        type=int,
        help="# steps to train on the dataSet",
    )
    ...
    parser.add_argument("--skip_head", default=SKIP_HEAD, type=int, help="skip head")
    parser.add_argument("--skip_tail", default=SKIP_TAIL, type=list, help="skip tail")
    ...
    parser.add_argument(
        ...
        default=GEOBOUNDARY,
        type=tuple,
        help="the boundary of the geometric data",
    )
    ...
    parser.add_argument(
        "--test-ratio", default=TEST_RATIO, type=float, help="the ratio of test case"
    )
    parser.add_argument(
        "--rand-seed",
        default=RAND_SEED,
...
def write_flags_and_BVE(flags, best_validation_loss):
    ...
    flags_dict_copy = (
        flags_dict.copy()
    )  # in order to not corrupt the original data strucutre
    flags_dict_copy["y_range"] = (
        yrange_str  # Change the y range to be acceptable long string
    )
