...
def training_from_flag(flags):
    ...
    dim_g = sample_data[0].shape[1]  # input dimension
    dim_s = sample_data[1].shape[1]  # output dimension

    ntwk = Network(
        dim_g=dim_g,
        dim_s=dim_s,
        linear=flags.linear,
        skip_connection=flags.skip_connection,
    ...
    ntwk.train_(
        train_loader,
        test_loader,
        epochs=flags.train_step,
        optm=flags.optim,
        weight_decay=flags.reg_scale,
        lr=flags.lr,
...
def importData(flags):
    ...
    for file_name in train_data_files:
        # import full arrays
        print(x_range)
        ftr_array = pd.read_csv(
            os.path.join(directory, file_name),
            delimiter=",",
            header=None,
            usecols=x_range,
        )
        lbl_array = pd.read_csv(
            os.path.join(directory, file_name),
            delimiter=",",
            header=None,
            usecols=y_range,
    ...
    for i in range(len(ftr[0, :])):
        print(
            "For feature {}, the max is {} and min is {}".format(
                i, np.max(ftr[:, i]), np.min(ftr[:, i])
            )
        )
